...
setwd("C:/Users/mreic/OneDrive/Dokumente/Masterarbeit/RTutor")
...
$\qquad$ $\qquad$    2.2 DiD - Example Germany
...
**Task:** Let us now take a look at the first few rows of `dat` using command `head()`.
```{r "2_a_2"}
head(dat)
```
...
The output shows data for the years 1987 to 1992. Overall, though, we want to observe the period from 1987 to 2009 for each bank. The data set therefore has a panel structure. The column `BankLoans` shows the total sum of bank loans of the bank in the respective year. When we talk about bank loans during this problem set, we will always refer to the total amount of loans (or a mathematically transformed value of this number) and not to individual loans or the total number of loans from a Bank. `BankLoans` as well as all other monetary variables are adjusted for inflation and measured in real 1995 euros. Furthermore, we can see the variable `StateID` which indicates in which state the bank is located. The states of Saxony and Saxony-Anhalt are not represented in the data set because the constituencies in these states have undergone several radical changes due to territorial reforms (cf. Englmaier & Stowasser, 2017, p.307). Therefore, we observe a total of 14 German states in our dataset. Bank $1$ is located in state $9$ which corresponds to Bavaria. We also see several other metrics which are introduced once they are used. 

**Task:** Let us start with a summary of the dataset. Press `check` to see how many savings and cooperative banks are in the sample as well as their average amount of bank loans in million euros per year.
```{r "2_a_3"}
#< task_notest
dat %>%
  group_by(BankType) %>%
...
**Task:** With this knowledge, let us take a closer look at the distribution of bank loans. Press `check`.
```{r "2_a_4",fig.width=9}
#< task_notest
...
```
#< quiz "distribution"
question: How would you describe the distribution of BankLoans based on this histogram?
sc:
    - skewed to the left
    - symmetric
    - skewed to the right*
...
**Task:** The table `dat` contains many variables that we are not interested in now. Hence, let us create the table `mdat` from `dat` with the variables `BankID`, `BankType`, `Year`, `Elec` and `BankLoans` using the command `select()`. Show `mdat`.

#< info "select"
  `select()` can be used to subset the collumns of a table.
 
  For example, this command returns only the columns `BankID` and `BankType` of the table `dat`:
```{r eval=FALSE}
  select(dat, BankID, BankType)
```
  You can find addidional information <a href="https://dplyr.tidyverse.org/reference/select.html" target="_blank">here</a> or by running the command `?select` in your R console.
#>

```{r "2_a_5"}
mdat <- select(dat, BankID, BankType, Year, Elec, BankLoans)
mdat
```
Table `mdat` is clearer than the entire data set and is consequently more suitable for the following operations. The variable `Elec` indicates whether a county election was held in the respective year. In the county where Bank 1 is located, elections were held in 1990, 1996, 2002 and 2008.

**Task:** Let us now create a table `onlySB` from `mdat` that only contains savings banks. Use the command `filter()` from package `dplyr`. Show 10 random rows of `onlySB` using `sample_n()`. 


#< info "filter"
  As the name implies, `filter()` can be used to filter a dataframe according to certain conditions. The output contains only those rows that satisfy all the conditions that were filtered for. Unlike `select()`, which creates a subset of columns, with `filter()` you create a subset of rows.
 
  You can filter by a single condition, such as a specific year:
```{r eval=FALSE}
  filter(dat, Year==1999)
```
  Multiple conditions can also be linked together using the operators `&` (AND), `|` (OR) and `xor()` (XOR). For example, the `filter()` command for savings banks in an election year with lending of more than 20 billion euros looks like this:
```{r eval=FALSE}
  dat %>% filter(BankType=='SB' & Elec==1 & BankLoans>2e10)
``` 
  In this example, only the observations that satisfy all three conditions are displayed. Note that the pipe operator `%>%` was used here. `%>%` is very useful to combine multiple commands. That way the output of the previous statement (here: `dat`) becomes the input of the following one (here: `filter(...)`.
  
  For additional information on `filter` click <a href="https://dplyr.tidyverse.org/reference/filter.html" target=" target="_blank">here</a> or run the command `?filter` in your R console.
#>

#< info "sample_n"
  Here is an example of how to use `sample_n()`:
```{r eval=FALSE}
  sample_n(dat,5)
```
  This command outputs 5 random rows from `dat`.
#>

```{r "2_b_2"}
onlySB <- mdat %>% filter(BankType=='SB')
sample_n(onlySB,10)
```
...
**Task:** Let us now pick out one bank (in this case, the one with `BankID` $6$) and have a look at that bank's lending over the 23-year period. The vertical dashed blue lines show the years in which an election was held in the bank's state.
```{r "2_c"}
#< task_notest
BankNo <- 6
...
#>
```
Several things can be gathered from the output of Bank 6. First, the bank is rather small. This can be seen from the low level of lending, which at around 500 million is much lower than the average lending of savings banks of over 1.3 billion. Moreover, lending tended to increase over the years. For example, lending increased by more than 50% between 1987 and 2009. And finally, the most important observation: Lending in election years appears to be higher compared to the other years. The amount of bank loans generally increased before an election and decreased thereafter. We will investigate whether this observation is statistically significant or just a coincidence in the next chapters.
...
**Task:** Press `edit` and `check` afterwards to load the required dataset.
```{r "3_1"}
#< task_notest
dat <- read.fst("dataset.fst")
#>
...
Before we go on to the analytical parts of this Problem Set, let us clarify which variables are of interest to us. Since we want to study the impact of elections on bank lending, our two variables of interest are the Election variable `Elec` and the measure for bank loans `BankLoans_Norm_LOG`. The variable `Elec` indicates whether an election was held in the state in which the bank is located in the corresponding year (`Elec` $=1$) or not (`Elec`$=0$). For bank loans, we do not use the absolute value `BankLoans`, but instead normalize that value with total bank assets `BankTotAss` and logarithmize it. The authors normalize `BankLoans` to account for bank size effects. Normalizing also reduces the skew in the distribution of the variable. The log transformation allows us to interpret the results more easily. More on this later.

**Task:** The transformation from `BankLoans` to `BankLoans_Norm_LOG` will be performed in this chapter. First, let us have a look at the two columns that are needed for the transformation. Press `check`.
```{r "3_a"}
#< task_notest
...
**Task:** Let us now create the column `BankLoans_Norm_LOG` in `dat` by normalizing the bank loans with the total assets and then logarithmizing this ratio. `BankLoans_Norm_LOG` therefore has the following value: $log(\frac{BankLoans}{BankTotAss})$. Use functions `mutate()` and `log()` to create the new variable.

#< info "mutate"
  The function `mutate()` from package `dplyr` can be used to add new variables to a data frame or to overwrite existing ones. 
 
  For example, a new column can be added to the data frame from the product of two existing columns. The following command adds/overwrites the variable `ElecxSB` with the product of `Elec` and `SB`. Thus, `ElecxSB` takes the value $1$ if both `Elec` and `SB` take the value $1$ - i.e., for savings banks in an election year.
```{r eval=FALSE}
dat <- dat %>%
  mutate(ElecxSB = SB * Elec)
``` 
You can find addidional information <a href="https://dplyr.tidyverse.org/reference/mutate.html" target="_blank">here</a> or by running the command `?mutate` in your R console.
#>

```{r "3_a_2"}
dat <- dat %>% mutate(BankLoans_Norm_LOG=log(BankLoans/BankTotAss))

#< hint
hint.stud.call(dat %>% mutate(BankLoans_Norm_LOG=log(BankLoans/BankTotAss)),"Great! The calculation is correct. But you also have to save the new dataset.")

auto.hint.else()
#>
```

**Task:** Let us have a look at the transformed variable `BankLoans_Norm_LOG`. Press `check`.
```{r "3_a_3"}
#< task_notest
head(select(dat, BankLoans, BankTotAss, BankLoans_Norm_LOG),10)
#>
```
Let us verify the value of `BankLoans_Norm_LOG` for the first observation: $5111808000/8678057600=0.58905$ and $log(0.58905)=-0.52924$. We conclude that the transformation seems to be correct.

**Task:** Finally, let us compare the distribution of `BankLoans_Norm_LOG` with the distribution of `BankLoans`. Press `check`.
```{r "3_a_4",fig.width=10}
#< task_notest
...
#< quiz "distribution of SBs and CBs"
question: On average, are SBs still larger than CBs in terms of transformed lending?
sc:
    - No, CBs are now larger than SBs.*
...
## Exercise 2 -- The Difference-in-Differences appoach (DiD) 
...
## Exercise 2.1 -- Introducing DiD - Example Bavaria

**Task:** Press `edit` and then `check` to load the required dataset.
```{r "4_1"}
#< task_notest
dat <- read.fst("dataset.fst")
#>
...
The aim of this chapter is to introduce the DiD approach. We first address the basic idea of the approach and then compute the DiD estimator for Bavaria only. In addition, we raise the question of whether a key assumption of the DiD approach is satisfied in this example.

**Task:** In this chapter we are only interested in data from Bavaria (state $9$). Let us therefore store all the data from the Bavarian banks in the table `bdat`. Use function `filter()`.
```{r "4_2"}
bdat <- dat %>%
  filter(StateID==9)
#< task
...
### group_by
`group_by` allows the user to convert a table into a grouped table. Operations can then be performed by group.
### summarise
With `summarise` all observations of each group can be summarized in any way. For example, you can calculate the mean, the maximum, or the number of observations of a variable by group. If the table is not grouped, all observations are summarized. A new data frame will be created.


To illustrate the two commands, let us now take a look at an example.
```{r eval=FALSE}
dat %>%
  group_by(StateID, BankType) %>%
  summarise(n=n_distinct(BankID))
``` 
The code chunk first groups the table `dat` by `StateID` and `BankType`. Then, for each combination of those two variables, the number of different `BankID`s is counted with the command `n_distinct`. We obtain a table containing the number of CBs and SBs for each state. So the first six lines of the output look like this
```
   StateID BankType     n
 1       1 CB          44
 2       1 SB          19
...
For addidional information on `summarise` click <a href="https://dplyr.tidyverse.org/reference/summarise.html" target="_blank">here</a> or run the command `?summarize` in your R console.
#>

**Task:** Now, calculate the average lending for both `BankType`s and for each `Year.` Fill in the blanks.
```{r "4_2"}
#< fill_in
# bav <- bdat %>%
#   ___(___, ___) %>%
...
#>
bav <- bdat %>%
  group_by(BankType, Year) %>%
  summarise(avg = mean(BankLoans_Norm_LOG, na.rm=TRUE))
```
**Task:** With the help of this table, we can now take a look at the lending activities of Bavarian banks. Let us start with the savings banks first and add the cooperative banks later. Press `check`.
```{r "4_2"}
#< task_notest
ggplot(data=filter(bav,BankType=="SB"), aes(x=Year, y=avg, color=BankType)) + 
  geom_line(size=0.8)+
  geom_point() +
...
**Task:** Calculate the average Bavarian savings banks lending in election years and non-election years using the table `bdat` and store the result in `bSB`. Remember: `bdat` already contains Bavarian banks only. Fill in the blanks. 
```{r "4_3"}
#< fill_in
# bSB <- bdat %>% 
#   filter(___)%>%
...
**Task:** So let us subtract the average of the non-election year from the average of the election year and store the value in the variable `DeltaSB`.
```{r "4_3"}
#< fill_in
# DeltaSB <- bSB$avg[___]-bSB$avg[___]
# round(DeltaSB, digits = 6)
#>
DeltaSB <- bSB$avg[2]-bSB$avg[1]
round(DeltaSB, digits = 6)
...
#< info "Linear approximation of the logarithm with the Taylor series"
  When $x$ takes a value close to $1$, $log(x)$ can be linearly approximated using the first-order Taylor polynomial.
  $$log(x)\overset{x~near~1}\approx\log(1)+\frac{log(1)^\prime}{1!} * (x-1)=0 + \frac{1}{1}*(x-1) = x-1$$
#>
...
#< quiz "Interpretation of the log difference"
question: So how can we interpret this? Average normalized lending is about 1.5 ...
sc:
    - ... percentage points higher in election years compared to non-election years
    - ... percentage points lower in election years compared to non-election years
    - ... percent higher in election years compared to non-election years*
    - ... percent lower in election years compared to non-election years
success: Thats true. 
failure: Try again.
...
**Task:** Before we delve deeper into the approach, let us take a look at the plot that shows both SB and CB lending in Bavaria. Press `check`.
```{r "4_4"}
#< task_notest
...
**Task:** Let us again check this observation by calculating the average lending in election years and non-election years. In this step we also calculate the difference between the two averages and store it in the variable `DeltaCB`. We will use this value again later. Press `check`.
```{r "4_4"}
#< task_notest
bCB <- bdat %>% 
  filter(BankType=='CB')%>%
...
We will now estimate the causal effect of county elections in Bavaria on Bavarian SB lending with the difference-in-differences estimator. 

**Task:** Let us first visualize how the DiD estimator is composed with the following illustration. The values in the graph do not match our example. Press `check`.
```{r "4_4"}
#< task_notest
ggplot()+ 
...
**Task:** Compute the value of the DiD estimator using the variables `DeltaSB` and `DeltaCB`. Fill in the blanks.
```{r "6_4"}
#< fill_in
# DiD = ___ - ___
# round(DiD, digits = 6)
...
## Exercise 2.2 --  DiD - Example Germany
...
**Task:** Let us start again with a chart showing the average lending of SBs and CBs of all German states. We also want to include elections in the plot. Since election years differ from state to state, it would not make any sense to display the plot in the same way as in the previous chapter. Instead, we plot each year with reference to the election year. For example, for `ElecWhen`$=-1$, we plot the average `BankLoans_Norm_LOG` of all banks one year ahead of an election, split into SB and CB. Press `check`.
```{r "4_5"}
#< task_notest
pdat <- dat %>%
...
#>
```
We see that savings bank lending is generally lower than those of cooperative banks. `plot1` further shows us that for cooperative banks, lending declines sharply in the election year and then rises again by about the same magnitude two years later. Thus, lending in the election year as well as in the year after is much lower than usual. This observation is surprising since elections should not have much of an impact on cooperative bank lending. In the next chapter, we will take a closer look at this observation in an excursus and examine how the financial crisis may have affected the graph.
...
#< quiz "causal effect plot1"
question: Based on `plot1`, what causal effect of elections on savings bank lending would you expect?
sc:
    - a negative one
    - around zero
    - a positive one*
...
We first must calculate the required mean values of those four subsets.
...
**Task:** To simplify the calculation, let us assign variable names to the average values. 
...
#>
y.noelec.co = mdat$mean[1]
y.noelec.tr = mdat$mean[3]
y.elec.co = mdat$mean[2]  
y.elec.tr = mdat$mean[4]
```

**Task:** Let us now calculate the DiD estimator with these 4 variables using the formula presented above. Fill in the blank.
```{r "6_4"}
#< fill_in
# DiD = ___
# round(DiD, digits = 6)
#>
DiD = (y.elec.tr-y.noelec.tr) - (y.elec.co-y.noelec.co)
round(DiD, digits = 6)
```
...
**Task:** Finally, here is a graph that further illustrates the interpretation of the DiD estimator in this example (Germany as a whole). Press `check`.
```{r "6_5"}
#< task_notest
...
#< task_notest
  ...
  ggplot(data=filter(b,StateID %in% c(5:8)), aes(x=Year, y=mean, color=BankType)) + 
  geom_line(size=0.8)+
...
#>
``` 
Now, if many states had an election in 2009, this drop in lending could have a big impact on `plot1`. 

**Task:** Let us examine how many banks are in states with an election in 2009.
```{r "5_2"}
#< task_notest
dat %>%
  filter(Year==2009) %>%
...
#< info "a look at the other years"
  - In 2007, elections were only held in Hamburg. This means that only for the six banks in Hamburg, 2009 is 2 years after the election. The exclusion of these 6 observations is not relevant for the overall average, which consists of almost 6000 observations for `ElecWhen`$=2$.
  - The values for `ElecWhen`$=-1$ and `ElecWhen`$=-2$ remain unchanged because the years 2010 and 2011 are not in the sample period. For banks with an election in 2010, 2009 would be the pre-election year. However, since 2010 does not appear in our dataset, omitting 2009 does not drop out any observations for that year. The same applies for 2011.
...
**Task:** To do this, we first create a table `deltadat` that contains the change in `BankLoans_Norm_LOG` from 2008 to 2009 for each bank. In addition, we store the banks' type as well as the information whether the bank is in a state where elections were held in 2009. Press `check`.
```{r "5_4"}
#< task_notest
deltadat <- dat %>%
  group_by(BankID) %>%
...
#>
```
The variable `Delta` captures the change in lending from 2008 to 2009. `Elecin2009` indicates whether the bank was in a state with elections in 2009.

**Task:** Now, we can calculate the average of `Delta` for each combination of `BankType` and `Elecin2009`.
```{r "5_4"}
#< task_notest
deltadat %>%
...
Basically, we are once again running a Difference in Differences analysis here, but with only one point in time. For cooperative banks the decline in lending from 2008 to 2009 was almost the same for banks with and without an election in 2009. For savings banks, on the other hand, the decline in lending is noticeably smaller for banks with an election in 2009 than for banks without an election in that year. Once again, this suggests that elections have a positive impact on SB lending. 

#< info "DiD estimation"
  The DiD estimator in this case has a value of:
  $$DID=(-0.0449736-(-0.05311233))-(-0.06458079-(-0.06517799))=0.00754153$$
  Thus, the 2009 election has a positive causal effect on the change in lending from 2008 to 2009 by savings banks of around $0.008$.
#>
...
A possible skewness triggered by the financial crisis can be taken into account in a regression by adding fixed effects of the variable `Year` (more on this later). For this reason, this problem is not considered hereafter.
...
#< quiz "variables"
question: Which of the variables is the dependent variable (Y-Value)?
sc:
    - SB
    - Elec
    - BankLoans_Norm_LOG*
...
We are mainly interested in the coefficient $\beta_3$ because it captures the causal effect of elections on savings bank lending. We will perform all regressions using the command `felm`. A detailed explanation of `felm` can be found in the Infobox.

#< info "felm"
The command `felm` from the `lfe` package can be used to estimate linear models. We can perform simple linear regressions with `felm` in the same way as with `lm`.  For example, we can regress the independent variable `Elec` on the dependent variable `BankLoans_Norm_LOG` with the following command:
```{r eval=FALSE}
felm(BankLoans_Norm_LOG ~ Elec, data  = dat)
``` 
For simple linear regressions like this, using `lm` would be more appropriate. However, the `felm` command can be easily extended using vertical dashes `|`. This allows us to include fixed effects, instrumental variables, and cluster robust standard errors in the regression. We are not dealing with instrumental variables in this thesis. The other two aspects will be addressed in exercise 3.2.

We can integrate them into the regression as follows:
```{r eval=FALSE}

felm(y ~ x1 + x2 | factors (fixed effects) | IV | cluster variable(s), data)
``` 
If certain parts of the formula are not used, you should replace them with the argument `0`. This is not necessary if those parts are at the end of the formula. In that case you can simply omit them. 

You can find addidional information <a href="https://www.rdocumentation.org/packages/lfe/versions/2.8-7/topics/felm" target="_blank">here</a> or by running the command `?felm` in your R console.
#>

#< info "Interaction in R"
To regress only the interaction of variables, separate them with `:`.

To regress the interaction of variables as well as the variables themselves, separate them with `*`.

Therefore, you can run the regression $y=\beta_0+\beta_1x_1+\beta_2x_2+\beta_3(x_1\times{x_2})$ in these two ways:
```{r eval=FALSE}
#option 1
felm(y ~ x1 + x2 + x1:x2 )
...
felm(y ~ x1*x2 )
```
Both options lead to the correct result in the next task.
#>
...
```{r "7_e"}
est <- felm(BankLoans_Norm_LOG ~ Elec + SB + Elec:SB, data = dat)

#< test_arg
other.sols = list(quote(
  est <- felm(BankLoans_Norm_LOG ~ Elec*SB, data = dat)
))
#>
```

**Task:** Let us have a look at the results. Press `check`.
```{r "7_e_2",results='asis'}
#< task_notest
stargazer(est,
          type="html",
          digits=4,
...
#< quiz "causal effect"
question: The coefficient of which variable reflects the causal effect of county elections on savings bank lending?
sc:
    - Elec
    - SB
    - Elec:SB*
...
**Task:** Let us now show how the other coefficients from `est` can be interpreted with the help of the following illustration. The illustration was already introduced in chapter 2.2. Press `check`.
```{r "7_e_3",results='asis'}
#< task_notest
mdat <- dat %>%
  filter(!is.na(Elec)) %>% 
  group_by(BankType,Elec)%>%
...
```
#< quiz "Beta 0"
question: What does the coefficient $\beta_0$ measure?
sc:
    - the average lending of CBs
    - the average lending of SBs
    - the average lending of CBs in non-election years*
    - the average lending of SBs in election years
success: Thats true. 
failure: Try again.
...
#< quiz "Beta 1"
question: What does the coefficient $\beta_1$ measure?
sc:
    - the gap between non-election year lending and election year lending of CBs*
    - the gap between non-election year lending and election year lending of SBs
    - the gap in lending between CBs and SBs in election years    
    - the gap in lending between CBs and SBs in non-election years
success: Thats true. 
failure: Try again.
...
#< quiz "Beta 2"
question: What does the coefficient $\beta_2$ measure?
sc:
    - the gap between non-election year lending and election year lending of CBs
    - the gap between non-election year lending and election year lending of SBs
    - the gap in lending between CBs and SBs in election years    
...
With this regression, we can estimate four different scenarios:
  ...
  - Our prediction of a savings bank's lending outside an election year is equal to $\hat{\beta}_{0}+\hat{\beta}_{2}$.
  - Our prediction of a savings bank's lending in an election year is equal to $\hat{\beta}_{0}+\hat{\beta}_{1}+\hat{\beta}_{2}+\hat{\beta}_{3}$.
...
In this chapter we will extend the simple regression from the previous chapter step by step. The goal is to end up with a complete regression that answers the main hypothesis of the paper. First, year and state fixed effects are included in the regression. Thereafter, a set of control variables for banks and counties is added. The authors' goal is to increase the predictability of `BankLoans_Norm_LOG` by adding these variables. They thereby also reduce the risk of omitted variable bias, which occurs when a variable that is correlated with the dependent variable (as well as with an explanatory variable) is omitted from the regression. As addressed, we hope that these variables partially explain the difference in lending behavior between SBs and CBs over time, thus weakening the parallel trends assumption. The last step is to cluster the standard errors.
...
with: 
  ...
  - $\delta$ is the vector of coefficients of the fixed effects. The interpretation of these values is not straightforward for year fixed effects. This is because that the DiD approach also aims to account for the development over time. The development over time is therefore taken into account twice.
...
#>
estFE <- felm(BankLoans_Norm_LOG~Elec*SB| StateID + Year , data = dat, keepX=TRUE)
```

**Task:** Let us compare the results of the regression with fixed effects with the results of the basic regression from chapter 2.4.
```{r "8_a 2",results='asis'}
#< task_notest
...
stargazer(est, estFE,
          ...
          omit.stat="ser",
          model.numbers=FALSE,
          column.labels=c("<pre>basic regression", "<pre>regression with FE"))
#>
```
We can see that the negative influence of elections on bank lending has already decreased noticeably compared to the basic regression. While $\hat{\beta}_{1}$ in the previous regression had the value $-0.014$, it is at $-0.006$ now, even though this value is no longer significant. The coefficient of `Elec:SB` $\hat{\beta}_{3}$, which is the most important for us, has increased slightly and remains significant at the 5% level. The coefficient of the intercept has disappeared from the regression and instead shows up as part of the fixed effects.


### Additional control variables
In the next task, two bank-specific metrics, total assets and equity ratio, are added. The two metrics are represented in our dataset by the variables `LAG_BankTotAss_LOG_QRT` and `LAG_BankCapRat_LOG_QRT`. We do not use the absolute values of the metrics, but their quartile indicators. This causes the variables to take only the values $1, 2, 3$ and $4$. The authors believe that the two variables affect lending and that their addition improves the regression.
...
#          omit.stat="ser",
#          model.numbers=FALSE,
#          column.labels=c("<pre>regression with bank controls"))
#>
estBM <- felm(BankLoans_Norm_LOG~Elec*SB + LAG_BankTotAss_LOG_QRT + LAG_BankCapRat_LOG_QRT| StateID+Year , data = dat, keepX=TRUE)
stargazer(estBM,
          type="html",
          digits=3,
          omit.stat="ser",
...
#          omit.stat="ser",
#          model.numbers=FALSE,
#          column.labels=c("<pre>regression with county controls"))
#>
estCM <- felm(BankLoans_Norm_LOG~Elec*SB + LAG_BankTotAss_LOG_QRT + LAG_BankCapRat_LOG_QRT + LAG_DistrGDP_LOG_QRT + LAG_DistrPop_LOG_QRT + LAG_DistrGDP_Gr_QRT + LAG_DistrPop_Gr_QRT | StateID+Year , data = dat, keepX=TRUE)
stargazer(estCM,
          type="html",
          digits=3,
          omit.stat="ser",
...
### Standard errors
The final operation performed in this chapter is the clustering of standard errors. If you are interested in the mathematical background to standard errors and cluster robust standard errors, check out the infobox below. There, we first discuss how standard errors are estimated in the general case and then address clustered standard errors.

#< info "standard errors and clustered standard errors"
Given a regression in the form of:  $y=X\beta+\varepsilon$

The **standard error of the regression** $\hat\sigma$ measures the accuracy of the regression and can be estimated using the following formula:
$$\hat\sigma=\sqrt{\frac{\sum_{i=1}^n \hat\varepsilon_i^2}{df}}$$
with $\hat\varepsilon_i$ being the residuals of the regression and $df$ being the degrees of freedom (# of observations - # of estimated parameters). Have a look at Wooldridge, 2016, p.88 for more information.

The **standard error of the coefficient** $\hat\beta_k$ indicates how accurate an estimator is. The variance of $\hat\beta$ is determined as follows:
$$Var(\hat\beta)=E(\hat\beta-E(\hat\beta))^2=(X^TX)^{-1}X^T\sigma^2IX(X^TX)^{-1}=\sigma^2(X^TX)^{-1}$$
The standard error of $\hat\beta_k$ is the square root of the kth diagonal element of this matrix (cf. Green, 2018, p.102). From this follows:
$$\widehat{SE}(\hat\beta_k)=\sqrt{[\hat\sigma^2(\mathbf{X}^T\mathbf{X})^{-1}]_{kk}}$$

To obtain the correct standard errors of the coefficients with this formula, the Gauss-Markov assumptions must be satisfied. Therefore, the error terms of all observations have to be **homoscedastic** ($Var(\varepsilon_i)=\sigma^2<\infty~~\forall{i}$) and have to have an **expected value of zero** ($E(\varepsilon_i)=0~~\forall{i}$). Furthermore, the error terms of different observations have to be **uncorrelated** ($Cov(\varepsilon_i,\varepsilon_j)=0~~\forall{i\neq{j}}$). However, the error terms of different observations are often correlated in some way, resulting in a biased standard error.

### Cluster robust standard errors
The authors suggest that this may be the case for banks located in the same region. If shocks occur at the regional level, for example due to the regional business cycle, there is a high probability that error terms of banks in the same geographic area are correlated. For this reason, the authors estimate **cluster robust standard errors**. That is, they assume that error terms of different observations within the same cluster are correlated. Error terms of different clusters, however, are still assumed to be uncorrelated (cf. Nichols, et. al., 2007, p.2). Cluster robust standard errors can be estimated as follows:

### Mathematical formula
Let us assume the sample consists of $C$ clusters labeled $c=1,...,C$. Each cluster consists of $N_c$ observations with $\sum_{c=1}^C{N_c}=n$. The regression is modeled as: $y_c=X_c\beta+\varepsilon_c$ 
 
Then, the variance of $\hat\beta$ is given by:
$$\widehat{Var}(\hat\beta)=(X^TX)^{-1} \mathbf{\sum_{c=1}^C(X_c^T\hat\varepsilon_c\hat\varepsilon_c^TX_c)}(X^TX)^{-1}$$
where $X_c$ is the $N_c\times{K}$ matrix of independent variables of cluster c and $\hat\varepsilon_c$ is the $N_c\times{1}$ vector of residuals of c (cf. Green, 2018, p.115).

Due to the finite number of clusters, the estimated value of the standard error is biased downward (cf. Cameron & Miller, 2015, p.324 f.). To reduce this bias, the formula can be adjusted as follows:
 $$\widehat{Var}(\hat\beta)=(X^TX)^{-1} (\frac{C}{C-1}\frac{n-1}{n-k}  \sum_{c=1}^CX_c^T\hat\varepsilon_c\hat\varepsilon_c^TX_c)(X^TX)^{-1}$$
 Standard errors in our regressions are estimated with this formula.
#>
...
#>
estSE <- felm(BankLoans_Norm_LOG~Elec*SB + LAG_BankTotAss_LOG_QRT + LAG_BankCapRat_LOG_QRT + LAG_DistrGDP_LOG_QRT + LAG_DistrPop_LOG_QRT + LAG_DistrGDP_Gr_QRT + LAG_DistrPop_Gr_QRT | StateID+Year | 0 | StateID, data = dat, keepX=TRUE)
```

**Task:** Let us look at the results of the regression with clustered standard errors and compare them to the previous regression. Press `check`.
```{r "8_d 2",results='asis'}
#< task_notest
...
#>
```
We see that the standard errors of the coefficients have changed. The standard error of the coefficient for `Elec:SB` decreases from $0.006$ to $0.004$. The value of the coefficient itself and the values of all other coefficients remain unchanged. This results in the coefficient of the DiD estimator now being significant at the 1% level. We obtain a positive causal effect of county elections on savings bank lending of 0.015 or 1.5%.
...
For the regression, the authors generated the variable `ElecCOUxSB` that should describe the interaction of `Elec` and `SB`. So, their regression formula looked like this: `BankLoans_Norm_LOG` ~ `Elec` + `SB` + `ElecCOUxSB` + ...

However, as I confirmed with the original authors an error occurred in the data encoding of `ElecCOUxSB` for the "Freie Sparkassen". As a reminder: these banks are called savings banks, but for our analyses they are to be treated in the same way as cooperative banks (their `SB` equals $0$). Yet `ElecCOUxSB` takes the value $1$ in election years for these banks, although it should always be 0. A total of 49 observations are affected. Since we use the interaction `Elec`*`SB` instead, our result is slightly different from the one in the paper and more precise.
#>

## Exercise 4 -- Additional analysis

During the problem set, we have already found out that county elections have a positive impact on savings bank lending in an election year. However, this is not the only finding from Englmaier and Stowasser's work. They ran lots of regressions to address various questions. Part of it is covered in more detail in this chapter.
...
**Task:** Let us start by loading our dataset. Press `edit` and then `check`.
```{r "10_1"}
#< task_notest
dat <- read.fst("dataset.fst")
#>
...
#< quiz "adjustment"
question: We now want to examine a year other than the election year. Which variables need to be adjusted?
sc:
    - the explained variable (BankLoans_Norm_LOG)
    - only Elec*
    - both Elec and SB
    - all explaining variables
success: That is correct, we just need to adjust the explanatory variable Elec to examine a year other than the election year.
failure: Try again.
...
**Task:** The other three required variables for $\tau=(-1,2,3)$ have already been generated. Let us take a look at them. Press `check`.
```{r "10_b_3"}
#< task_notest
...
**Task:** Let us start with the year before an election. Adjust the given regression analyzing the election year in a way that it represents the pre-election year. The required election-indicator for this is `PreElec`.
```{r "10_c"}
#< fill_in
...
#>
#Modify the regression in such a way that it maps the pre-election year
est_lead <- felm(BankLoans_Norm_LOG ~ PreElec*SB + LAG_BankTotAss_LOG_QRT + LAG_BankCapRat_LOG_QRT + LAG_DistrGDP_LOG_QRT + LAG_DistrPop_LOG_QRT + LAG_DistrGDP_Gr_QRT + LAG_DistrPop_Gr_QRT | StateID + Year | 0 | StateID, data = dat, keepX=TRUE)

```

**Task:** Let us repeat this modification for the post-election year. Use the variable `PostElec`.
```{r "10_c 2"}
#< fill_in
...
#>
#Modify the regression in such a way that it maps the post-election year
est_lag1 <- felm(BankLoans_Norm_LOG ~ PostElec*SB + LAG_BankTotAss_LOG_QRT + LAG_BankCapRat_LOG_QRT + LAG_DistrGDP_LOG_QRT + LAG_DistrPop_LOG_QRT + LAG_DistrGDP_Gr_QRT + LAG_DistrPop_Gr_QRT | StateID + Year | 0 | StateID, data = dat, keepX=TRUE)

```

**Task:** The regressions for the other years of the electoral cycle are already given. Just press `check`.
```{r "10_c 3"}
#< task_notest
...
**Task:** Now let us look at the results.
```{r "10_c 4",results='asis'}
#< task_notest
...
```
### Interpretation
In addition to the already known results from the election year, we obtain significant results for the year before and the second year after the election. Both findings are significant at the 10% level. The result indicates that elections have a positive causal effect on savings bank lending in the pre-election year, with a coefficient of 0.012. This means that, ceteris paribus, an election in the following year increases the savings bank's normalized lending in the current year by an average of 1.2%. A negative relationship between elections and savings bank lending is found for the second year after the election. A coefficient of -0.015 means that our prediction for normalized lending is reduced by over 1.5% if the observed bank is a savings bank two years after an election. No significant results were found for the year after an election and the year three years after an election.

### Conclusion
The results of these regressions indicate that the increase in savings bank lending is likely to already start in the pre-election year. The assumption that lending declines again after the election can also be confirmed at the 10% level for the second year after the election. These conclusions are only accurate if all our assumptions for the regressions are met.
...
In addition to the overall effect of elections on savings bank lending, the authors also examined the role that the contestedness of an election plays in this effect. 

#< quiz "contestedness"
question: What do you think? Is election-induced lending higher or lower for close elections compared to elections in general?
sc:
    - higher*
    - lower
...
We think that politicians are more likely to increase savings bank lending when a close result is anticipated in the run-up to the election. If incumbents knew in advance that they were certain to win or lose the election, it would be unnecessary for them to increase lending. However, if a close election outcome is expected in advance, additional votes are more valuable because they could decide the outcome of the election. In this case, politicians have a higher incentive to increase savings bank lending.
...
**Task:** Let us first apply the regression to the subset of contested elections. You can create such a subset with the command `filter()` (see Exercise 1). `ElecCloseIndex3_MED` indicates whether the election was contested ($1$) or not ($0$). Fill in the blank.
```{r "11_a"}
#< fill_in
# #Contested election
...
#>
#Contested election
estC <- felm(BankLoans_Norm_LOG ~ Elec*SB + LAG_BankTotAss_LOG_QRT + LAG_BankCapRat_LOG_QRT + LAG_DistrGDP_LOG_QRT + LAG_DistrPop_LOG_QRT + LAG_DistrGDP_Gr_QRT + LAG_DistrPop_Gr_QRT | StateID + Year | 0 | StateID, data = filter(dat, ElecCloseIndex3_MED==1), keepX=TRUE)

```

**Task:** The code for uncontested elections is already given. Press `check`.
```{r "11_a"}
#< task_notest
...
**Task:** Lets now have a look at the results. Press `check`.
```{r "11_a_2",results='asis'}
#< task_notest
...
**Task:** The indicator variables required for this have already been generated. Let us have a look at them. Press `check`.
```{r "12_d"}
#< task_notest
...
#>
estDef01 <-felm(BankLoansBadNorm_LOG ~ ElecPost01*SB + LAG_BankTotAss_LOG_QRT + LAG_BankCapRat_LOG_QRT + LAG_DistrGDP_LOG_QRT + LAG_DistrPop_LOG_QRT + LAG_DistrGDP_Gr_QRT + LAG_DistrPop_Gr_QRT | StateID + Year | 0 | StateID, data =dat, keepX=TRUE)
```
**Task:** The regressions for the other three year bundles work in the same way. We use the variables `ElecPost12`, `ElecPost23` and `ElecPost34` as election indicators. The code for the other three regressions is already given. Press `check`.
```{r "12_d_3"}
#< task_notest
#tau=1/2
...
#>
```
**Task:** Let us now take a look at the results. Press `check`.
```{r "12_d_4",results='asis'}
#< task_notest
...
**Task:** The regressions are identical to those from above, except that we filter the data set by `ElecCloseIndex3_MED` and `ElecIncTenure_MED_ALT`. The code for the regressions is already given. Press `check`.
```{r "12_d_3"}
#< task_notest
#tau=0/1
...
#>
```
We obtain significant results for all four year bundles. Accordingly, elections reduce the loan default rate by 0.6% in the election year and the year after. Moreover, elections reduce the loan default rate by 0.3% in years 1 and 2 after an election. The results for $\tau=(2,3)$ and $\tau=(3,4)$ confirm our hypothesis that loan default rates increase towards the end of the election cycle. In years 2 and 3 after an election, nonperforming loans increase by 0.4%. In years 3 and 4 after an election, this coefficient is significant at the 1% level and stands at 1%.

So elections, for which we expect to see a lot of election-induced lending, have a larger impact on the loan default rate than elections in general.
...
### ElecIncTenure_MED
Let us first look at the variable `ElecIncTenure_MED`, which the authors use. The variable is equal to 1 if the current ruling party has been in power for 14 years or more and 0 if not. What happens if the ruling party at the time of the election has been in power for more than 14 years and loses the election? Then the variable ElecIncTenure_MED takes the value 0 already in the election year and the observation would thus not be part of our calculation. However, the observation should be included in the specification (given the election is additionally contested), since the party responsible for the election-induced lending was already in power for more than 14 years.

### ElecIncTenure_MED_ALT
The basic structure of `ElecIncTenure_MED_ALT` is the same as that of `ElecIncTenure_MED`: it has the value 1 if the current ruling party has been in power for 14 years or more and 0 if not. For the scenario described above, in which the former ruling party had already been in power for 14 years and lost the election, the two variables differ. In this case, the variable `ElecIncTenure_MED_ALT` takes the value $1$ for the entire next election cycle. These observations are not excluded from the regression since the party responsible for the election-induced lending was already entrenched.

For a better understanding, here is an example that illustrates the differences between the two variables.

```
   BankID  Year  Elec ElecPost01 ElecIncTenure ElecIncTenure_MED ElecIncTenure_MED_ALT
 1      1  1987     0          0            17                 1                     1
 2      1  1988     0          0            18                 1                     1
 3      1  1989     0          0            19                 1                     1
 4      1  1990     1          1             0                 0                     1
...
12      1  1998     0          0             2                 0                     0
```
In the 1990 election year, the ruling party had already been in office for 20 years and was thus well entrenched. Therefore, `ElecIncTenure_MED_ALT` is equal to 1 for the entire subsequent election cycle, even though the election was lost.
#>
