...
**Task:** Let us start with a brief summary of the dataset. Press `check` to see how many savings and cooperative banks are in the sample as well as their average amount of bank loans in million euros per year.
```{r "2_a_3"}
#< task_notest
dat %>%
  group_by(BankType) %>%
...
**Task:** The table `dat` contains many variables that we are not interested in at the moment. Hence, let us create the table `mdat` from `dat` with the variables `BankID`, `BankType`, `Year`, `Elec` and `BankLoans` using the command `select()`. Show `mdat`.
...
``` 
  ...
  For addidional information on `filter` click <a href="https://dplyr.tidyverse.org/reference/filter.html" target=" target="_blank">here</a> or run the command `?filter` in your R console.
#>
...
**Task:** Let us now pick out one particular bank (in this case, the one with `BankID` $6$) and have a look at that bank's lending over the 23-year period. The vertical dashed blue lines show the years in which an election was held in the bank's state.
```{r "2_c"}
#< task_notest
BankNo <- 6
...
**Task:** Let us again check this observation by calculating the average lending in election years and non-election years. In this step we also calculate the difference between the two averages and store it in the variable `DeltaCB`. We will use this value again later on. Press `check`.
```{r "4_4"}
#< task_notest
bCB <- bdat %>% 
  filter(BankType=='CB')%>%
...
**Task:** Let us examine how many banks are located in states with an election in 2009.
```{r "5_2"}
#< task_notest
dat %>%
  filter(Year==2009) %>%
...
**Task:** To do this, we first create a table `deltadat` that contains the change in `BankLoans_Norm_LOG` from 2008 to 2009 for each bank. In addition, we store the banks' type as well as the information whether the bank is located in a state where elections were held in 2009. Press `check`.
```{r "5_4"}
#< task_notest
deltadat <- dat %>%
  group_by(BankID) %>%
...
**Task:** Now let us take a look at the results.
...
### ElecIncTenure_MED
Let us first take a look at the variable `ElecIncTenure_MED`, which the authors use. The variable is equal to 1 if the current ruling party has been in power for 14 years or more and 0 if not. What happens if the ruling party at the time of the election has been in power for more than 14 years and loses the election? Then the variable ElecIncTenure_MED takes the value 0 already in the election year and the observation would thus not be part of our calculation. However, the observation should actually be included in the specification (given the election is additionally contested), since the party responsible for the election-induced lending was already in power for more than 14 years.
